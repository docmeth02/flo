  ...
  class CarPlayCoordinator {
    ...
    private func makeLibraryTab() -> CPListTemplate {
      let playSomethingItem = CPListItem(
        text: String(localized: "Play Something"), detailText: nil,
      ...
      let section = CPListSection(
        items: [playSomethingItem, albumsItem, artistsItem, songsItem, likedSongsItem])
        ...
        DispatchQueue.main.async {
          ...
          case .failure:
            let errorItem = CPListItem(
              ...
              detailText: String(localized: "Tap to retry"))
    ...
    private func showArtistsList() {
      ...
      AlbumService.shared.getArtists { [weak self] result in
        DispatchQueue.main.async {
          ...
          case .failure:
            let errorItem = CPListItem(
              text: String(localized: "Failed to load artists"),
              detailText: String(localized: "Tap to retry"))
            errorItem.handler = { [weak self] _, completion in
              self?.interfaceController.popTemplate(animated: false, completion: nil)
              self?.showArtistsList()
    ...
    private func playArtistRadio(artist: Artist) {
      RadioService.shared.getSimilarSongs(id: artist.id) { [weak self] result in
        DispatchQueue.main.async {
          ...
          case .success:
            self.showErrorTemplate(
              title: String(localized: "No Radio Available"),
              message: String(localized: "No similar songs found for \(artist.name)."))
          case .failure:
            self.showErrorTemplate(
              title: String(localized: "Radio Unavailable"),
              message: String(localized: "Could not load radio for \(artist.name)."))
          }
        }
      }
    }
    ...
    private func playArtistTopSongs(artist: Artist) {
      RadioService.shared.getTopSongs(artistName: artist.name, count: 20) { [weak self] result in
        DispatchQueue.main.async {
          ...
          case .success:
            self.showErrorTemplate(
              title: String(localized: "No Top Songs"),
              message: String(localized: "No top songs found for \(artist.name)."))
          case .failure:
            self.showErrorTemplate(
              title: String(localized: "Unavailable"),
              message: String(localized: "Could not load top songs for \(artist.name)."))
          }
        }
      }
    }
    ...
    private func showSongsList() {
      ...
      AlbumService.shared.getAllSongs { [weak self] result in
        DispatchQueue.main.async {
          ...
          case .failure:
            let errorItem = CPListItem(
              text: String(localized: "Failed to load songs"),
              detailText: String(localized: "Tap to retry"))
            errorItem.handler = { [weak self] _, completion in
              self?.interfaceController.popTemplate(animated: false, completion: nil)
              self?.showSongsList()
    ...
    private func showLikedSongs() {
      ...
      AlbumService.shared.getStarredSongs { [weak self] result in
        DispatchQueue.main.async {
          ...
          case .failure:
            let errorItem = CPListItem(
              text: String(localized: "Failed to load liked songs"),
              detailText: String(localized: "Tap to retry"))
            errorItem.handler = { [weak self] _, completion in
              self?.interfaceController.popTemplate(animated: false, completion: nil)
              self?.showLikedSongs()
          ...
          case .success(let albums):
            ...
            if filtered.isEmpty && cachedSongs.isEmpty {
              template.updateSections([
                CPListSection(items: [
                  CPListItem(
                    text: String(localized: "No downloads"),
                    detailText: String(localized: "Download music from the app"))
                ])
              ])
              return
            }
    ...
    private func showCachedSongs(songs: [Song]) {
      ...
      let template = CPListTemplate(
        title: String(localized: "Cached"), sections: [actionSection, trackSection])
      interfaceController.pushTemplate(template, animated: true, completion: nil)
    }
  }
#endif
